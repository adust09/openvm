...
pub extern "C" fn zkvm_sha256_impl(prev_state: *const u8, input: *const u8, output: *mut u8) {
    // SAFETY: assuming safety assumptions of the inputs, we handle all cases where `prev_state` or
...
pub extern "C" fn zkvm_sha512_impl(prev_state: *const u8, input: *const u8, output: *mut u8) {
    // SAFETY: assuming safety assumptions of the inputs, we handle all cases where `prev_state` or
    // `input` or `output` are not aligned to 4 bytes.
    // The minimum alignment required for the buffers
    const MIN_ALIGN: usize = 4;
    unsafe {
        let prev_state_is_aligned = prev_state as usize % MIN_ALIGN == 0;
...
#[inline(always)]
#[no_mangle]
pub extern "C" fn zkvm_sha384_impl(prev_state: *const u8, input: *const u8, output: *mut u8) {
    // SAFETY: assuming safety assumptions of the inputs, we handle all cases where `prev_state` or
    // `input` or `output` are not aligned to 4 bytes.
    // The minimum alignment required for the buffers
    const MIN_ALIGN: usize = 4;
    unsafe {
        ...
        __native_sha384_compress(prev_state_ptr, input_ptr, output_ptr);
...
#[cfg(target_os = "zkvm")]
#[inline(always)]
fn __native_sha384_compress(prev_state: *const u8, input: *const u8, output: *mut u8) {
    openvm_platform::custom_insn_r!(opcode = OPCODE, funct3 = SHA2_FUNCT3, funct7 = Sha2BaseFunct7::Sha384 as u8, rd = In output, rs1 = In prev_state, rs2 = In input);
}
