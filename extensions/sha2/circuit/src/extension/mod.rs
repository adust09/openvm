...
impl<F> VmExecutionExtension<F> for Sha2 {
    ...
    fn extend_execution(
        ...
        inventory.add_executor(sha384_executor, [Rv32Sha2Opcode::SHA384.global_opcode()])?;
...
impl<SC: StarkGenericConfig> VmCircuitExtension<SC> for Sha2 {
    fn extend_circuit(&self, inventory: &mut AirInventory<SC>) -> Result<(), AirInventoryError> {
        ...
        // the block hasher chip needs its own bus to interact within itself
        let sha2_block_hasher_bus_index = inventory.new_bus_idx();
        ...
        // SHA-384
        let sha384_main_air = Sha2MainAir::<Sha384Config>::new(
            inventory.system().port(),
            bitwise_lu,
            inventory.pointer_max_bits(),
            sha2_bus_index,
        ...
        inventory.add_air(sha384_main_air);

        let sha384_block_hasher_air = Sha2BlockHasherVmAir::<Sha384Config>::new(
            bitwise_lu,
            sha2_block_hasher_bus_index,
            sha2_bus_index,
        );
        inventory.add_air(sha384_block_hasher_air);
    ...
    Val<SC>: PrimeField32,
{
    fn extend_prover(
        ...
        // the arena will be passed to the block hasher chip by the main chip
        let arena = Arc::new(Mutex::new(None));
        let sha256_block_hasher_chip = Sha2BlockHasherChip::<Val<SC>, RA, Sha256Config>::new(
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
            arena.clone(),
        ...
        let sha256_main_chip = Sha2MainChip::<Val<SC>, RA, Sha256Config>::new(
            arena,
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
        ...
        // the arena will be passed to the block hasher chip by the main chip
        let arena = Arc::new(Mutex::new(None));
        let sha512_block_hasher_chip = Sha2BlockHasherChip::<Val<SC>, RA, Sha512Config>::new(
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
            arena.clone(),
        ...
        let sha512_main_chip = Sha2MainChip::<Val<SC>, RA, Sha512Config>::new(
            arena,
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
        ...
        // SHA-384
        inventory.next_air::<Sha2BlockHasherVmAir<Sha384Config>>()?;
        // the arena will be passed to the block hasher chip by the main chip
        let arena = Arc::new(Mutex::new(None));
        let sha384_block_hasher_chip = Sha2BlockHasherChip::<Val<SC>, RA, Sha384Config>::new(
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
            arena.clone(),
        );
        inventory.add_periphery_chip(sha384_block_hasher_chip);

        inventory.next_air::<Sha2MainAir<Sha384Config>>()?;
        let sha384_main_chip = Sha2MainChip::<Val<SC>, RA, Sha384Config>::new(
            arena,
            bitwise_lu.clone(),
            pointer_max_bits,
            mem_helper.clone(),
        );
        inventory.add_executor_chip(sha384_main_chip);
