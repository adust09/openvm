...
use crate::{
    add_padding_to_message, write_slice_to_memory, Sha2BlockHasherChip, Sha2BlockHasherVmAir,
    Sha2BlockHasherVmConfig, Sha2BlockHasherVmDigestColsRefMut, Sha2Config, Sha2MainAir,
    Sha2MainChip, Sha2MainChipConfig, Sha2VmExecutor, SHA2_WRITE_SIZE,
};

const SHA2_BUS_IDX: BusIndex = 28;
const SHA2_BLOCK_HASHER_BUS_IDX: BusIndex = 29;
type F = BabyBear;
const MAX_INS_CAPACITY: usize = 4096;
type Harness<RA, C> =
    TestChipHarness<F, Sha2VmExecutor<C>, Sha2MainAir<C>, Sha2MainChip<F, RA, C>, RA>;

fn create_harness_fields<RA: Arena, C: Sha2Config>(
    system_port: SystemPort,
    bitwise_chip: SharedBitwiseOperationLookupChip<RV32_CELL_BITS>,
    memory_helper: SharedMemoryHelper<F>,
    pointer_max_bits: usize,
) -> (Sha2MainAir<C>, Sha2VmExecutor<C>, Sha2MainChip<F, RA, C>) {
    let executor = Sha2VmExecutor::<C>::new(Rv32Sha2Opcode::CLASS_OFFSET, pointer_max_bits);
    let empty_arena = Arc::new(Mutex::new(None));
...
fn create_test_harness<RA: Arena, C: Sha2Config>(
    ...
    (Sha2BlockHasherVmAir<C>, Sha2BlockHasherChip<F, RA, C>),
) {
    ...
    let block_hasher_air =
        Sha2BlockHasherVmAir::new(bitwise_chip.bus(), SHA2_BLOCK_HASHER_BUS_IDX, SHA2_BUS_IDX);
    let empty_arena = Arc::new(Mutex::new(None));
    ...
    (
        ...
        (block_hasher_air, block_hasher_chip),
    )
}
...
fn set_and_execute_single_block<RA: Arena, C: Sha2Config, E: PreflightExecutor<F, RA>>(
    ...
    match C::VARIANT {
        Sha2Variant::Sha256 => {
            assert_eq!(
                expected_output,
                tester.read::<{ Sha256Config::DIGEST_BYTES }>(RV32_MEMORY_AS as usize, dst_ptr)
            );
        }
        Sha2Variant::Sha512 => {
            assert_eq!(
                expected_output,
                tester.read::<{ Sha512Config::DIGEST_BYTES }>(RV32_MEMORY_AS as usize, dst_ptr)
            );
        }
        Sha2Variant::Sha384 => {
            assert_eq!(
                expected_output,
                tester.read::<{ Sha384Config::DIGEST_BYTES }>(RV32_MEMORY_AS as usize, dst_ptr)
            );
        }
    }
}
...
fn rand_sha2_single_block_test<C: Sha2Config + 'static>() {
    ...
    let (mut harness, bitwise, block_hasher) = create_test_harness::<_, C>(&mut tester);
...
fn set_and_execute_full_message<RA: Arena, C: Sha2Config + 'static, E: PreflightExecutor<F, RA>>(
    ...
    let message = add_padding_to_message::<C>(message);

    // run SHA2_UPDATE as many times as needed to hash the entire message
    message.chunks_exact(C::BLOCK_BYTES).for_each(|block| {
...
///////////////////////////////////////////////////////////////////////////////////////
fn sha2_edge_test_lengths<C: Sha2Config + 'static>(test_vectors: &[(&str, &str)]) {
    let mut rng = create_seeded_rng();
    let mut tester = VmChipTestBuilder::default();
    let (mut harness, bitwise, block_hasher) = create_test_harness::<_, C>(&mut tester);
...
fn sha256_edge_test_lengths() {
    ...
    sha2_edge_test_lengths::<Sha256Config>(&test_vectors);
}
...
fn execute_roundtrip_sanity_test<C: Sha2Config + 'static>() {
    let mut rng = create_seeded_rng();
    let mut tester = VmChipTestBuilder::default();
    let (mut harness, bitwise, block_hasher) =
...
fn negative_sha2_test_bad_final_hash<C: Sha2Config + 'static>() {
    let mut tester = VmChipTestBuilder::default();
    let (harness, bitwise, block_hasher) =
        create_test_harness::<MatrixRecordArena<F>, C>(&mut tester);
    ...
    let modify_trace = |trace: &mut RowMajorMatrix<F>| {
        trace.row_chunks_exact_mut(1).for_each(|row| {
            ...
            let mut cols = Sha2BlockHasherVmDigestColsRefMut::from::<C>(
                &mut row_slice[..<C as Sha2BlockHasherVmConfig>::DIGEST_WIDTH],
            );
            if cols.inner.flags.is_last_block.is_one() && cols.inner.flags.is_digest_row.is_one() {
