...
impl MultiRowMetadata for Sha2Metadata {
    ...
    fn get_num_rows(&self) -> usize {
        // the record arena associated with the SHA-2 executor will be re-used to store the trace
        // of the block hasher chip, so we need to return the number of rows that the block hasher
        // chip will need.
        match self.variant {
    ...
    for<'buf> RA: RecordArena<'buf, Sha2RecordLayout, Sha2RecordMut<'buf>>,
{
    ...
    fn execute(
        ...
        debug_assert!(
            record.inner.dst_ptr as usize + C::DIGEST_BYTES <= (1 << self.pointer_max_bits)
        );
        debug_assert!(
        ...
        let mut hash_state = record.prev_state.to_vec();
        C::compress(&mut hash_state, record.message_bytes);
        ...
        for idx in 0..C::DIGEST_WRITES {
            tracing_write::<SHA2_WRITE_SIZE>(
                ...
                hash_state[idx * SHA2_WRITE_SIZE..(idx + 1) * SHA2_WRITE_SIZE]
                    .try_into()
                    .unwrap(),
