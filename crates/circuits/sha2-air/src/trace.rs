...
impl<C: Sha2BlockHasherSubairConfig> Sha2BlockHasherFillerHelper<C> {
    ...
    pub fn generate_block_trace<F: PrimeField32>(
        ...
        for (i, row) in trace.chunks_exact_mut(trace_width).enumerate() {
            // do the rounds
            if i < C::ROUND_ROWS {
                let mut cols: Sha2RoundColsRefMut<F> =
                    Sha2RoundColsRefMut::from::<C>(&mut row[..C::ROUND_WIDTH]);
            ...
            // generate the digest row
                ...
                let mut cols: Sha2DigestColsRefMut<F> =
                    Sha2DigestColsRefMut::from::<C>(&mut row[..C::DIGEST_WIDTH]);
        ...
        for i in 0..C::ROWS_PER_BLOCK - 1 {
            ...
            let mut local_cols: Sha2RoundColsRefMut<F> =
                Sha2RoundColsRefMut::from::<C>(&mut local[..C::ROUND_WIDTH]);
            let mut next_cols: Sha2RoundColsRefMut<F> =
                Sha2RoundColsRefMut::from::<C>(&mut next[..C::ROUND_WIDTH]);
            if i > 0 {
                for j in 0..C::ROUNDS_PER_ROW {
                    next_cols
                        .schedule_helper
    ...
    /// next block
    pub fn generate_missing_cells<F: PrimeField32>(&self, trace: &mut [F], trace_width: usize) {
        ...
        let mut cols_last_round_row: Sha2RoundColsRefMut<F> =
            Sha2RoundColsRefMut::from::<C>(&mut last_round_row[..C::ROUND_WIDTH]);
        let mut cols_digest_row: Sha2RoundColsRefMut<F> =
            Sha2RoundColsRefMut::from::<C>(&mut digest_row[..C::ROUND_WIDTH]);
        let mut cols_next_block_first_row: Sha2RoundColsRefMut<F> =
            Sha2RoundColsRefMut::from::<C>(&mut next_block_first_row[..C::ROUND_WIDTH]);
    ...
    pub fn generate_default_row<F: PrimeField32>(&self, mut cols: Sha2RoundColsRefMut<F>) {
        ...
        for i in 0..C::ROUNDS_PER_ROW {
            set_arrayview_from_u32_slice(
                ...
                mem::take(&mut hash[C::ROUNDS_PER_ROW - i - 1]).into_iter(),
            );
            set_arrayview_from_u32_slice(
                &mut cols.work_vars.e.row_mut(i),
                mem::take(&mut hash[C::ROUNDS_PER_ROW - i + 3]).into_iter(),
            );

            set_arrayview_from_u32_slice(
                &mut cols.work_vars.carry_a.row_mut(i),
                C::get_invalid_carry_a(i).into_iter().cloned(),
            );
            set_arrayview_from_u32_slice(
                &mut cols.work_vars.carry_e.row_mut(i),
                C::get_invalid_carry_e(i).into_iter().cloned(),
            );
        }
    }
...
pub fn generate_trace<F: PrimeField32, RA: Send + Sync, C: Sha2BlockHasherSubairConfig>(
    filler_helper: &Sha2BlockHasherFillerHelper<C>,
    bitwise_lookup_chip: SharedBitwiseOperationLookupChip<8>,
    width: usize,
    ...
    struct BlockContext<C: Sha2BlockHasherSubairConfig> {
    ...
    for (input, is_last_block) in records {
        ...
        if is_last_block {
            ...
            prev_hash = Sha2BlockHasherFillerHelper::<C>::get_block_hash(&prev_hash, input);
        }
    }
    // first pass
    values
        ...
        .for_each(|(block, ctx)| {
            ...
            let input_words = (0..C::BLOCK_WORDS)
                ...
                .collect::<Vec<_>>();
            filler_helper.generate_block_trace(
                block,
                width,
                &input_words,
    ...
    values[width * non_padded_height..]
        ...
        .for_each(|row| {
            ...
            filler_helper.generate_default_row(cols);
        });
    ...
    values[width..]
        ...
        .for_each(|chunk| {
            filler_helper.generate_missing_cells(chunk, width);
        });
